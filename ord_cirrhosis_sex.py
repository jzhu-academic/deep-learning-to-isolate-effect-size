...
df = pd.concat([df, dummies_sex, dummies_dru], axis=1)
df.drop(["Sex", "Drug"], axis=1, inplace=True)
...
#                 print(i, loss5.data.item(), accuracy(pred(y_logits5), batch_y).item(), accuracy(pred(model5(X_test))).item())
    ...
    y_harmful = torch.full([len(X_f)], 2).to(device)
    X_f = torch.from_numpy(np.array(X_f)).float().to(device)
    female_risk = accuracy(combine5_pred(X_f), y_harmful)
    X_m = torch.from_numpy(np.array(X_m)).float().to(device)
    male_risk = accuracy(combine5_pred(X_m), y_harmful)
    return [accuracy(combine5_pred()).item(), female_risk.item(), male_risk.item()]
...
filepath = Path(f"./ord_{hid1}_{lr}_{momentum}_{epochs}.csv")
